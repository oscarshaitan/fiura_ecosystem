        ...
        child: BlocBuilder<JudgeCubit, JudgeState>(
          builder: (context, state) {
            return state.maybeWhen(
              orElse: () => const Center(
                child: Text("Ups! Ocurrió un error mientras cargabamos este jurado"),
              ),
              loading: () => const Center(child: OnLoadMessage()),
              error: (message) => Center(child: Text(message)),
              loadJudge: (judge) {
                return FutureBuilder<void>(
                  future: precacheImage(NetworkImage(judge.urlPhoto), context),
                  builder: (BuildContext context, AsyncSnapshot<void> snapshot) {
                    if (snapshot.connectionState == ConnectionState.done) {
                      if (snapshot.hasError) {
                        return Center(
                          child: Text('Ups! Ocurrió un error mientras cargabamos este jurado: ${snapshot.error}'),
                        );
                      } else {
